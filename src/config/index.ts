...
export const config: Config = {
  ...
  taskSimulatedDuration: parseInt(
    process.env.TASK_SIMULATED_DURATION || '2000',
    10
  ),
  taskSimulatedErrorPercentage: parseInt(
    process.env.TASK_SIMULATED_ERROR_PERCENTAGE || '20',
    10
  ),
  taskErrorRetryDelay: parseInt(
    process.env.TASK_ERROR_RETRY_DELAY || '1000',
    10
  ),
...
if (
  config.taskSimulatedErrorPercentage < 0 ||
  config.taskSimulatedErrorPercentage > 100
) {
